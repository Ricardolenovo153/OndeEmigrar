...
SET @max_score = (
    SELECT MAX((IFNULL(gdp_per_capita, 0)/1000*20) + (IFNULL(life_expectancy, 0)*20) + (IFNULL(tertiary_education, 0)*2*20) + (IFNULL(political_stability, 0)*10*15) + (IFNULL(rule_oflaw, 0)*10*15) - (IFNULL(share, 0)*5*10))
...
SELECT 
    country.country_name, 
    ROUND((
        (IFNULL(indicator.gdp_per_capita, 0) / 1000 * 20) + 
        (IFNULL(indicator.life_expectancy, 0) * 20) + 
        (IFNULL(indicator.tertiary_education, 0) * 2 * 20) + 
        (IFNULL(indicator.political_stability, 0) * 10 * 15) + 
        (IFNULL(indicator.rule_oflaw, 0) * 10 * 15) - 
        (IFNULL(indicator.share, 0) * 5 * 10)
    ), 0) AS score_final,
    ROUND((
        (IFNULL(indicator.gdp_per_capita, 0) / 1000 * 20) + 
        (IFNULL(indicator.life_expectancy, 0) * 20) + 
